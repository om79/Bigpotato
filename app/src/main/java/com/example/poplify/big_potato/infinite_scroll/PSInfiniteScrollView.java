					...
					((Activity)context).runOnUiThread(new Runnable() {
						public void run() {
							Collections.sort(carouselItems,
									new Comparator<View>() {
										...
										public int compare(View lhs,
												View rhs) {
											LayoutParams lhsParams = (LayoutParams) lhs
													.getLayoutParams();
											LayoutParams rhsParams = (LayoutParams) rhs
													.getLayoutParams();
							...
							for (int i = 0; i < carouselItems.size(); i++) {
								...
								LayoutParams params = (LayoutParams) vw
										.getLayoutParams();
								if (i == 0) {
									params.leftMargin += scrollX;
								} else {
									LayoutParams fparams = (LayoutParams) carouselItems
								...
								if (vw != replicaF && vw != replicaB) {
									if (scrollX > 0) {
										...
										if (params.leftMargin >= (itemCount * itemSize.width)) {
											LayoutParams fparams = (LayoutParams)replicaF.getLayoutParams();
